...
export function mapRuntimeOrder(row) {
  ...
  return {
    ...
    reason: row.reason || row.active_stop_reason || null,
    stop_notes: row.stop_notes || row.active_stop_notes || null,
...
export function deriveRuntimeOrders(baseOrders, sessions = [], stops = [], lowEffLogs = []) {
  ...
  return (baseOrders || []).map((order) => {
    ...
    return mapRuntimeOrder({
      ...
      reason: activeStop?.reason || null,
      stop_notes: activeStop?.notes || null,
