...
export default function Pet04({
  ...
  useEffect(() => {
    if (!ativosP4) return;
    setAtiva(ativosP4[0] ? { ...ativosP4[0] } : null);
    setProximo(ativosP4[1] ? { ...ativosP4[1] } : null);
  }, [ativosP4]);
      ...
      <div className={`pet01-card ${pet01StatusClass(ativa?.status)}`}>
        ...
        <div className={statusClass(ativa?.status)}>
          <Etiqueta o={ativa} variant="pet01" saldoCaixas={saldo} lidasCaixas={lidas} />
                  ...
                  <div className="stop-reason-below-P4">{stopReason}</div>
                  )}
        </div>
  ...
  <div className="pet01-modal-bg" role="dialog" aria-modal>
    <div className="pet01-modal">
      ...
      <p style={{ marginTop: 4, color: '#444' }}>
        Informe o operador responsável da P4 para o Turno {shiftInfo?.shiftKey || ""}. Este passo é obrigatório no início do turno.
      </p>
