...
const lastFinalizadoPorMaquina = useMemo(() => {
  ...
  if (location && location.pathname === '/pet-01') {
    // Estado para garantir atualização instantânea do Pet01
    const [ativosP1, setAtivosP1] = useState([]);
    useEffect(() => {
      setAtivosP1(ordens.filter(o => o.machine_id === 'P1' && !o.finalized).sort((a,b)=>(a.pos??999)-(b.pos??999)));
    ...
    return (
      <>
        ...
        {/* MODAIS - mesmos do fluxo principal */}
  ...
  // rota oculta: /pet-02 — renderiza diretamente a tela Pet01 (registroGrupos já existe aqui)
  if (location && location.pathname === '/pet-02') {
    // Estado para garantir atualização instantânea do Pet01
    const [ativosP2, setAtivosP2] = useState([]);
    useEffect(() => {
      setAtivosP2(ordens.filter(o => o.machine_id === 'P2' && !o.finalized).sort((a,b)=>(a.pos??999)-(b.pos??999)));
    }, [ordens, finalizadas, paradas]);
    return (
      <>
        <Pet02
  ...
  // rota oculta: /pet-03 — renderiza diretamente a tela Pet01 (registroGrupos já existe aqui)
  if (location && location.pathname === '/pet-03') {
    // Estado para garantir atualização instantânea do Pet01
    const [ativosP3, setAtivosP3] = useState([]);
    useEffect(() => {
      setAtivosP3(ordens.filter(o => o.machine_id === 'P3' && !o.finalized).sort((a,b)=>(a.pos??999)-(b.pos??999)));
    }, [ordens, finalizadas, paradas]);
    return (
      <>
        <Pet03
